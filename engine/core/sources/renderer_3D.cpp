...
//            window->Clear();
        ...
        void Renderer_3D::endFrame(){
            ...
            for(RenderCommand& command: m_RenderQueue){
                renderMesh(*command.mesh, command.transform);
            }
            m_RenderQueue.clear(); // Queue leeren nach dem Rendern
        }

        void Renderer_3D::present() {
            std::swap(m_FrameBufferFront,m_FrameBufferBack);
            window->swapBuffers(m_FrameBufferFront);
            window->Present();
            clearBuffer();
        }

        void Renderer_3D::renderMesh(const Mesh& mesh, const mat4& transform){
//            MVP-Matrix:
//            Model: command.transform
//            View: camera.getViewMatrix
//            Projection: camera.getProjectionMatrix
//            mat4 mvpMatrix = camera->getProjectionMatrix() * (camera->getViewMatrix() * command.transform);
            mat4 viewModel = camera->getViewMatrix() * transform;
...
//                 DEBUG_PRINT("MVP: \n" <<mvpMatrix);
        ...
        void Renderer_3D::rasterizeMesh(const Mesh& mesh, const mat4& mvpMatrix) {
            ...
            // hier dem bufferRenderer den buffer, w und h übergeben
            renderer2D.setTargetBuffer(m_FrameBufferBack.data(), m_width, m_height);
...
//                std::cout << "drawing wireframe from triangle " <<   i << "\n";
                renderer2D.drawTriangleWireFrame(screenCoords[0], screenCoords[1], screenCoords[2], renderColor);
...
//                std::this_thread::sleep_for(std::chrono::milliseconds(1000));
        ...
        void Renderer_3D::clearBuffer() {
            std::fill(m_FrameBufferBack.begin(), m_FrameBufferBack.end(), backGroundColor);
            std::fill(m_DepthBufferBack.begin(), m_DepthBufferBack.end(), 1.0f);       // Maximaler Tiefenwert
        }
